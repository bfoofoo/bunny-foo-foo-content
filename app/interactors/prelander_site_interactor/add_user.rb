module PrelanderSiteInteractor
    class AddUser
      ...
      delegate :prelander_site, :params, :request, :user, :prelander_site_user, :to => :context
  
      def call
        create_user
        create_prelander_site_user
        context.api_response = {user: user, is_verified: prelander_site_user&.is_verified, prelander_site_user: prelander_site_user}
      end
      ...
      def prelander_site_service
        @prelander_site_service ||= FormsiteService.new
      end
  
      def is_useragent_valid
        prelander_site_service.is_useragent_valid(request.user_agent)
      end
  
      def is_impressionwise_test_success
        return false if user.blank?
        prelander_site_service.is_impressionwise_test_success(user)
      end
  
      def is_email_duplicate
        return false if user.blank?
        !prelander_site.prelander_site_users.joins(:user).where("users.email = ?", user.email).blank?
      end
  
      def is_ip_duplicate?
        !prelander_site.prelander_site_users.where(ip: user_ip).blank?
      end
      ...
      def create_prelander_site_user
        if user.try(:email) == PrelanderSiteUser::TEST_USER_EMAIL
          create_test_user
        else
      ...
      def handle_site_user_creation
        ...
        if user.blank?
          if !is_ip_duplicate?
            ...
            context.prelander_site_user = prelander_site.prelander_site_users.find_by(ip: user_ip, user_id: nil)
          end
        else
          prelander_site_user = prelander_site.prelander_site_users.find_by(ip: user_ip, user_id: nil)
