ActiveAdmin.register Website do
  ...
  form do |f|
    tabs do
      ...
      tab 'QUESTIONS' do
        f.has_many :questions, new_record: true do |ff|
          ff.input :position 
          ff.input :text
          ff.input :_destroy, :as => :boolean, :required => false, :label => 'Remove'
          ff.input :flow, as: :select, collection: Question.flows.keys, include_blank: false 
          ff.has_many :answers do |fff|
            fff.input :text 
            fff.input :redirect_url
          end
        end
      end
