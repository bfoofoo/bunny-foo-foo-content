  ...
  controller do
    ...
    def index
      respond_to do |format|
        format.html
        format.csv { send_data @questions_statistics.stats_to_csv(affiliate: true), filename: "subaffiliate_#{@questions_statistics.formsite.name}_#{params[:start_date]}_#{params[:end_date]}.csv" }
      end
    end
  end
